...
test_that("subtotal_row outputs are consistent", {
  ...
  expect_snapshot(error = TRUE, subtotal_row(df_bad, mtcars, vars = "mpg"))
})

test_that("subtotal_row advanced scenarios", {
  ...
  expect_snapshot(subtotal_row(df_iris, iris, vars = c("Sepal.Length", "Sepal.Width")))
  ...
  expect_snapshot(subtotal_row(df_dia, ggplot2::diamonds, vars = "price", aggregator = "mean", exclude = 1))
  ...
  expect_snapshot(subtotal_row(df_cars, mtcars, vars = "mpg",
  ...
  expect_snapshot(subtotal_row(df_na, airquality, vars = "Ozone"))
  ...
  expect_snapshot(subtotal_row(df_complex, mtcars,
                                   vars = c("mpg", "disp"),
                                   aggregator = "median",
                                   exclude = c(2, 3)))
  ...
  expect_snapshot(subtotal_row(df_breaks, warpbreaks,
                                   vars = "breaks",
                                   aggregator = "sd"))
  ...
  expect_snapshot(subtotal_row(df_dates,
  ...
  expect_snapshot(subtotal_row(df_weird_groups,
...
test_that("Snapshot: High cardinality, multiple variables, default aggregator", {
  ...
  expect_warning({
    out <- subtotal_row(report, frame, vars = c("sales", "profit"))
...
test_that("Snapshot: Exclude parameter, median aggregator, uneven distribution", {
  ...
  out <- subtotal_row(report, frame, vars = "salary", aggregator = "median")
...
test_that("Snapshot: Deep grouping (4 levels), custom function, missing values", {
  ...
  expect_warning({
    out <- subtotal_row(report, frame, vars = c("uptime", "latency"), aggregator = "max")
...
#
# subtotal_row(report, frame, vars = c("uptime", "latency"), aggregator = "max")
# subtotal_row(report, frame, vars = c("uptime", "latency"), aggregator = "custom_max")

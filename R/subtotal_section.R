...
#'
#' @description Adds subtotal sections to a data report. Designed to work with reports generated by group_by + summarise.
...
#' @return The grouped data report with subtotal sections included.
#' @export
#'
#' @examples
#' library(dplyr)
...
subtotal_section <- function(report, frame, vars = "Population", aggregator = "sum", exclude = numeric(0),
    ...
    if (exclude_length == length(label_cols)){
      ...
      for (e in exclude){
        exclude_vec <- lapply(include_vec, function(x){
          min(ifelse(e %in% x & !((e + 1) %in% x), 0, 1))})
  ...
  for (j in 1:length(include_vec)){
    ...
    if (length(agg_parameter) == 0){
      ...
      mutate_report <- tryCatch({group_by(mutate_frame, across(all_of(label_cols))) %>% summarise(
      across(all_of(vars), function(x){rlang::exec(aggregator, x, !!!agg_parameter)}), .groups = "drop")},
